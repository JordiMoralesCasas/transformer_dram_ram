...
class MultiNumberTrainer:
    ...
    def __init__(self, config, train_loader=None, val_loader=None, test_loader=None, is_gridsearch=False):
        """
        ...
        self.image_size = 54 if self.snapshot else 224
    ...
    def validate(self, epoch):
        ...
        with tqdm(total=self.num_valid) as pbar:
            for i, batch in enumerate(self.valid_loader):
                ...
                pbar.set_description(
                    (
                        "VALIDATION: {:.1f}s - loss: {:.3f} - acc: {:.3f}".format(
                            (time.time() - tic), loss.item(), acc.item()
                        )
                    )
                )
                pbar.update(self.batch_size)
    ...
    def test(self):
        ...
        with tqdm(total=self.num_test) as pbar:
            for i, batch in enumerate(self.test_loader):
                ...
                batch_ids = batch.id.to(self.device)
                ...
                for i in range(self.batch_size):
                    results.append({
                        ...
                        "sample_id": batch_ids[i].tolist()
                    })
                    
                # update progress bar
                pbar.set_description(
                    (
                        "TEST: {:.1f}s - acc: {:.3f}".format(
                            (time.time() - tic), acc.item()
                        )
                    )
                )
                pbar.update(self.batch_size)
